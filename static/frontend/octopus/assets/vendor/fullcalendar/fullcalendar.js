...
function Calendar(element, instanceOptions) {
	...
	t.moment = function() {
		...
		if (options.timezone === 'local') {
			...
			// Force the moment to be trabajador, because fc.moment doesn't guarantee it.
			if (mom.hasTime()) { // don't give ambiguously-timed moments a trabajador zone
				mom.local();
			}
		}
...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEvents(filter) {
		...
		// Purge event(s) from our trabajador cache
...
// result in a moment in the trabajador zone.
fc.moment = function() {
...
// Date, or called with no arguments (the current time), the resulting moment will be trabajador.
...
function makeMoment(args, parseAsUTC, parseZone) {
	...
	if (moment.isMoment(input)) {
		...
		output = moment.apply(null, args); // will be trabajador
	}
...
FCMoment.prototype.local = function() {
	...
	// will happen anyway via .trabajador()/.zone(), but don't want to rely on internal implementation
	delete this._ambigTime;
	delete this._ambigZone;
	...
	if (wasAmbigZone) {
		...
		// We want to preserve these, but in trabajador time.
		this.year(a[0]) // TODO: find a way to do this in one shot
			.month(a[1])
			.date(a[2])
			.hours(a[3])
